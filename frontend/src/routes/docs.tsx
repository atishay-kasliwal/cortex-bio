        ...
        <main className="space-y-16">
          <header>
            ...
            <p className="mt-3 max-w-2xl text-muted-foreground">
              REST API for cognitive readiness, performance forecasts, chronotype, and recovery
              telemetry from any supported wearable.
            </p>
          </header>
          ...
          <DocBlock id="intro" title="Introduction">
            ...
            <p>
              OpenAPI:{" "}
              <a href={`${DOCS_API}/openapi.json`} className="underline">
                {DOCS_API}/openapi.json
              </a>{" "}
              · Interactive docs:{" "}
              <a href={`${DOCS_API}/docs`} className="underline">
                {DOCS_API}/docs
              </a>
            </p>
          </DocBlock>
          ...
          <DocBlock id="auth" title="Authentication">
            ...
            <p>
              ...
              or <code className="font-mono text-xs">cb_test_…</code>
            </p>
          ...
          <DocBlock id="quickstart" title="Quick Start">
            <ol className="list-decimal space-y-2 pl-5">
              ...
              <li>
                Call <code className="font-mono text-xs">GET /v1/readiness/today</code> with your
                key or session JWT
              </li>
            </ol>
            <p className="mt-4">
              SDKs: <code className="font-mono text-xs">@cortexbio/sdk</code> (npm) and{" "}
              <code className="font-mono text-xs">cortexbio</code> (PyPI)
            </p>
          </DocBlock>
          ...
          <DocBlock id="explorer" title="API Explorer">
            {spec.isLoading ? (
              ...
              <p>
                ...
                <button type="button" className="underline" onClick={() => spec.refetch()}>
                  Retry
                </button>
              </p>
            ) : (
                  ...
                  <div key={path} className="rounded-lg border border-border p-3 font-mono text-xs">
                    ...
                    <div className="mt-1 text-muted-foreground">
                      {Object.entries(methods).map(([method, meta]) => (
                        <span key={method} className="mr-3 uppercase">
                          {method}: {meta.summary ?? meta.description ?? "—"}
                        </span>
                      ))}
                    </div>
                  </div>
          ...
          <DocBlock id="limits" title="Rate Limits">
            <p>
              Free tier: 60 requests/minute per API key. JWT dashboard sessions: 120 requests/minute
              per user.
            </p>
          </DocBlock>
          ...
          <DocBlock id="errors" title="Error Codes">
            <p>
              <code className="font-mono">401</code> Unauthorized ·{" "}
              <code className="font-mono">404</code> No data ·{" "}
              <code className="font-mono">429</code> Rate limited ·{" "}
              <code className="font-mono">501</code> Provider not implemented
            </p>
          </DocBlock>

          <p className="text-sm text-muted-foreground">
            Need an API key?{" "}
            <Link to="/dashboard/api-keys" className="text-foreground underline">
              Open the dashboard
            </Link>
            .
          </p>
