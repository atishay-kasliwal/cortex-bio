...
function ForecastPage() {
  ...
  const cards =
    forecast.data && windows.data ? forecastWindowCards(forecast.data, windows.data) : [];
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Hourly cognitive performance for the next 24 hours, with detected work windows."
      />
      <div className="rounded-xl border border-border bg-card p-6">
        <div className="h-[420px]">
          {performanceCurve.length === 0 ? (
            <div className="flex h-full items-center justify-center text-sm text-muted-foreground">
              No forecast data yet. Connect a wearable and sync.
            </div>
          ) : (
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {(cards.length
          ? cards
          : [
              { label: "Peak", time: "—", val: 0 },
              { label: "Secondary", time: "—", val: 0 },
              { label: "Trough", time: "—", val: 0 },
            ]
        ).map((w) => (
