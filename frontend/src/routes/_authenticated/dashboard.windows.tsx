...
function WindowsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Work windows"
        description="Personalized blocks of time tuned to your biology, today."
      />
      <div className="grid gap-4 md:grid-cols-2">
        {isLoading ? (
          ...
          <p className="text-sm text-muted-foreground">
            No windows computed yet. Sync wearable data first.
          </p>
        ) : (
          windows.map((w) => (
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="mt-2 text-3xl font-semibold tracking-tight">
                    {w.start} <span className="text-muted-foreground">→</span> {w.end}
                  </div>
                </div>
                <div className="flex h-12 w-12 items-center justify-center rounded-lg border border-border bg-muted/40 font-mono text-sm font-medium">
                  {w.score}
                </div>
              </div>
